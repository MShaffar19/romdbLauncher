...
namespace file
{
	...
	std::vector<char> readBytes(const std::wstring& filePath);

	std::string readText(const std::string& filePath);
	std::string readText(const std::wstring& filePath);

	void writeBytes(const std::string& filePath, const char* data, size_t size);
	void writeBytes(const std::wstring& filePath, const char* data, size_t size);

	void writeText(const std::string& filePath, const std::string& str);
	void writeText(const std::wstring& filePath, const std::string& str);
	...
	// get the program's path without the program's file name
	std::filesystem::path getProgramPath();

	// get the user's  roaming path
	std::filesystem::path getUserRoamingPath();
}
