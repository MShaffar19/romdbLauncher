...
std::vector<SystemMediaFiles> Romdb::searchMedia(const std::string& search, size_t index)
{
	std::vector<SystemMediaFiles> searchResults;
	searchMedia(search, index, searchResults);
	return searchResults;
}

void Romdb::searchMedia(const std::string& search, size_t index, std::vector<SystemMediaFiles>& searchResults)
{
	if (!db)
		return;
	...
	for (const auto& system : query(*db, "SELECT id, name, code FROM system"))
	{
		...
		for (const auto& media : qry)
		{
			...
			query qry2(*db, "SELECT id, name FROM file WHERE media_id = :media_id");
