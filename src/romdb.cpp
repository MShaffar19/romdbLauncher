...
std::vector<SystemMediaFiles> Romdb::searchMedia(const std::string& search, size_t index, bool getArchives)
{
	std::vector<SystemMediaFiles> searchResults;
	searchMedia(search, index, getArchives, searchResults);
	return searchResults;
}

void Romdb::searchMedia(
	const std::string& search, size_t index, bool getArchives, std::vector<SystemMediaFiles>& searchResults)
{
	...
	for (const auto& system : query(*db, "SELECT id, name, code FROM system"))
	{
		SystemMediaFiles smf;
		smf.index = index;
		smf.system.id = system.get<long long>(0);
		smf.system.name = system.get<std::string>(1);
		smf.system.code = system.get<std::string>(2);

		query qry(*db, "SELECT id, name FROM media WHERE system_id = :system_id AND name LIKE :search COLLATE NOCASE");
		qry.bind(":system_id", smf.system.id);
		qry.bind(":search", search, nocopy);
		for (const auto& media : qry)
		{
			MediaFiles mf;
			mf.media.id = media.get<long long>(0);
			mf.media.name = media.get<std::string>(1);

			query qry2(*db);
			if (getArchives)
				qry2.prepare("SELECT id, name FROM file WHERE media_id = :media_id");
			else
				qry2.prepare("SELECT id, name FROM file WHERE media_id = :media_id AND (compression IS NULL OR "
							 "compression <> 'archive')");
			qry2.bind(":media_id", mf.media.id);
			for (const auto& file : qry2)
			{
				File f;
				f.id = file.get<long long>(0);
				f.name = file.get<std::string>(1);
				mf.files.push_back(std::move(f));
			}
			smf.mediaFiles.push_back(std::move(mf));
		}
		searchResults.push_back(smf);
	}
}

void Romdb::getSystems(std::vector<std::pair<std::string, std::string>>& systems)
{
	if (!db)
		return;

	for (const auto& system : query(*db, "SELECT name, LOWER(code) FROM system"))
	{
		auto code = system.get<std::string>(1);
		bool found = false;
		...
		if (!found)
		{
			auto name = system.get<std::string>(0);
			systems.push_back(std::make_pair(code, name));
		}
	}
}
