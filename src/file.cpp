...
std::vector<char> file::readBytes(const std::wstring& filePath)
{
	HANDLE hFile =
	...
	if (hFile)
	{
		...
		std::vector<char> bytes((size_t)fSize);
		ReadFile(hFile, bytes.data(), bytes.size(), NULL, NULL);
		CloseHandle(hFile);
		return bytes;
	}
	return {};
...
std::string file::readText(const std::wstring& filePath)
{
	auto bytes = readBytes(filePath);
	return std::string(bytes.data(), bytes.size());
}
...
void file::writeBytes(const std::wstring& filePath, const char* data, size_t size)
{
	HANDLE hFile =
		CreateFileW(filePath.c_str(), GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hFile)
	{
...
void file::writeText(const std::wstring& filePath, const std::string& str)
{
	writeBytes(filePath, str.data(), str.size());
}
...
std::filesystem::path file::getProgramPath()
{
...
std::filesystem::path file::getUserRoamingPath()
{
